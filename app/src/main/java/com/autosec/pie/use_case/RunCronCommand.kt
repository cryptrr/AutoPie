...
class RunCronCommand(private val processManagerService: ProcessManagerService){
    operator fun invoke(item: CommandModel, commandExtraInputs: List<CommandExtraInput> = emptyList(), processId: Int) : Flow<CommandResult> {
        ...
        return flow {
            ...
            val processResult = processManagerService.runCommandForShareWithEnv2(item, fullExecPath, resultCommand,path ,inputParsedData,commandExtraInputs,"",processId, JobType.CRON ,usePython, isShellScript)

            val jobKey = commandExtraInputs.map{it.value}.joinToString(" : ")

            val result = processResult.toCommandResult(JobType.CRON, jobKey)
